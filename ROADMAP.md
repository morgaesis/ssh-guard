...
- Session observability and caller execution identity: `guard session show <token>` now reports bounded in-memory interaction history, allow/deny and exec outcome counts, source breakdown, and an LLM risk histogram. Root-owned Unix-socket daemons can opt into `--exec-as-caller` so approved commands run as the connecting UID instead of the daemon identity.
